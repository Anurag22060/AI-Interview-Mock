...
const Feedback = ({ params }) => {
  ...
  const GetFeedback = async () => {
    ...
    if (result.length > 0) {
      ...
      calculateOverallRating(result); // Calculate the overall rating
    } else {
  ...
  const calculateOverallRating = (feedback) => {
    const totalRating = feedback.reduce((acc, item) => acc + parseInt(item.rating), 0);
    const avgRating = totalRating / feedback.length;
    setOverallRating(avgRating.toFixed(1)); // Set the overall rating with one decimal point
  };
