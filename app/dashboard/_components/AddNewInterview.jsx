...
const AddNewInterview = () => {
  ...
  const onSubmit = async (e) => {
    ...
    const InputPrompt =
      ...
      " , Years of Experience: " +
      jobExperience +
      " , Depending on this information please provide me " +
      process.env.NEXT_PUBLIC_INTERVIEW_QUESTION_COUNT +
      " interview questions with answers in JSON Format, Give Question and Answered as field in JSON";

    // const result = await chatSession.sendMessage(InputPrompt); // send prompt to AI
    // console.log("Gemini raw result:", result);

    // const rawText = result.output?.[0]?.content?.[0]?.parts?.[0]?.text || "";
